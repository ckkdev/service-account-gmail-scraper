...
parent_dir = os.path.join(str(Path(os.getcwd())) ,'EMAILS')
...
def run_on_email(email_user, account_scope, service_account_json):
...
def AccountServiceScraper(email_users, account_scope, service_account_json):
    ...
    for email_user in email_users:
        try:
            run_on_email(email_user, account_scope, service_account_json)
